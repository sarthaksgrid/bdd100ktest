...
# SCRIPT_DIR is models/DETR/
SCRIPT_DIR = Path(__file__).resolve().parent
# ROOT_DIR is the project root (up two levels from models/DETR)
ROOT_DIR = SCRIPT_DIR.parent.parent

# NEW: Explicitly define the local checkpoint directory for this script
# This ensures checkpoints are saved in /models/DETR/checkpoints/
SCRIPT_CHECKPOINT_DIR = SCRIPT_DIR / "checkpoints"

# Inject project root into sys.path so we can import 'utils'
if str(ROOT_DIR) not in sys.path:
    sys.path.append(str(ROOT_DIR))

# Import central configuration from project root
...
def main():
    ...
    print(f"[PATHS] Checkpoint Directory: {SCRIPT_CHECKPOINT_DIR}")
    ...
    # Locate split indices in data/splits/
    ...
    SCRIPT_CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    checkpoint_files = list(SCRIPT_CHECKPOINT_DIR.glob("detr_epoch_*.pt"))
    
    if checkpoint_files:
        latest_ckpt = max(checkpoint_files, key=lambda p: int(p.stem.split('_')[-1]))
        print(f"[RESUME] Found local checkpoint: {latest_ckpt.name}. Loading...")
    ...
    for epoch in range(start_epoch, target_epochs):
        ...
        plt.legend()
        plt.grid(True)
        # Saves plot inside models/DETR/checkpoints/
        plt.savefig(SCRIPT_CHECKPOINT_DIR / "loss_curve.png") 
        plt.close() 

        # Save Checkpoint to local folder
        ckpt_path = SCRIPT_CHECKPOINT_DIR / f"detr_epoch_{current_epoch}.pt"
        torch.save({
            'epoch': current_epoch,
        ...
        if current_epoch == target_epochs:
            torch.save(model.state_dict(), SCRIPT_CHECKPOINT_DIR / "final_m4_pro_model.pt")
            print(f"\n[SUCCESS] Final model saved to {SCRIPT_CHECKPOINT_DIR / 'final_m4_pro_model.pt'}")
